...
class Reservation extends Component {
  ...
  render() {
    const { step, deptSeats, arrSeats, showModal, activeStep, skipped } =
      this.state;
    ...
    const handleNext = () => {
      if (this.state.activeStep === 1) {
        ...
        if (
          this.state.arrSeats.length < Number(adults) + Number(children) ||
          this.state.deptSeats.length < Number(adults) + Number(children)
        ) {
    ...
    const { deptFlight, arrFlight, deptCabin, arrCabin, totalPrice } =
      sessionStorage.getItem("deal");
    return (
      <>
        <div
          style={{ backgroundColor: "background-color: rgba(0, 0, 0, 0.575)" }}
        >
          <MyHeader />
          <Box sx={{ width: "100%" }}>
            <Stepper
                ...
                {activeStep == 0 ? (
                  <div style={{ height: "59vh" }}>
                    ...
                    <ConfirmBooking
                      ...
                      parentFunc={() => this.handleSignedIn()}
                      deptSeats={deptSeats}
                      arrSeats={arrSeats}
                      totalPrice={totalPrice}
                    />
                  ...
                  <div style={{ height: "59vh" }}>
                    ...
                    <ChooseSeats
                      ...
                      deptCabin={deptCabin}
                      arrCabin={arrCabin}
                      parentFunc={(deptSeats, arrSeats) =>
                        this.handleSeatsConfirm(deptSeats, arrSeats)
                      }
                    />
                  </div>
                ) : activeStep == 2 ? (
          ...
          >
            ...
            <Modal.Body>
              <h5>
                ...
                <strong>{this.state.resId}</strong>
              </h5>
            </Modal.Body>
          </Modal>
        </div>
        ...
        <Modal
          ...
          onHide={this.handleSeatWarningShow.bind(this)}
